...
function privateIp(address) {
  ...
  if (family === 4) {
    ...
    return a===10 || a===127 || a===0 || (a===169&&b===254) || (a===172&&b>=16&&b<=31) ||
      (a===192&&b===168) || (a===100&&b>=64&&b<=127) || a>=224 || (a===192&&b===0&&c===2) ||
...
async function safeRssUrl(raw) {
  ...
  if(!['http:','https:'].includes(url.protocol)||url.username||url.password) throw new Error('URLが許可されていません');
  if(url.port&&!['80','443'].includes(url.port)) throw new Error('ポートが許可されていません');
  const host=url.hostname.replace(/\.$/,'');
  if(host==='localhost'||host.endsWith('.local')||host.endsWith('.internal')) throw new Error('ローカルURLは取得できません');
  if(net.isIP(host)) {
    if(privateIp(host)) throw new Error('非公開IPは取得できません');
  } else {
    const addresses=await dns.lookup(host,{all:true,verbatim:true});
    if(!addresses.length||addresses.some(item=>privateIp(item.address))) throw new Error('安全な公開アドレスではありません');
  }
  return url;
}
...
function rssHeaders(profile, url) {
  if(profile===1) {
    return {
  ...
  return {
    ...
    'User-Agent':'PersonalDashboardV2/2.16.0 (+RSS reader; iPhone PWA)',
    'Accept-Language':'ja,en-US;q=0.7,en;q=0.5'
  };
}

async function fetchRssWithRedirects(initialUrl, timeoutMs, profile) {
  let url=await safeRssUrl(initialUrl.href || initialUrl);
  for(let index=0;index<=RSS_REDIRECTS;index+=1) {
    const response=await fetch(url,{
      redirect:'manual',
      headers:rssHeaders(profile,url),
...
async function handleRss(req,res) {
  try {
    ...
    const timeoutMs=Math.max(3000,Math.min(12000,Number.isFinite(requested)?requested:12000));
    const source=String(first(req.query?.source)||'').trim().toLowerCase();
    const raw=KNOWN_RSS[source] || String(first(req.query?.url)||'');
    if(!raw) return res.status(400).json({ok:false,error:'RSS URLを指定してください'});
    const initial=await safeRssUrl(raw);

    let lastStatus=0;
    for(const profile of [0,1]) {
      const {response,url}=await fetchRssWithRedirects(initial,timeoutMs,profile);
      lastStatus=response.status;
      if(response.ok) {
        const body=await readRss(response);
        const head=body.subarray(0,1200).toString('utf8');
        if(!/<(?:rss|feed|rdf:RDF)\b/i.test(head)) {
          if(profile===0) continue;
          throw new Error('取得先がRSSではなくHTMLを返しました');
        }
        res.setHeader('Content-Type','application/xml; charset=utf-8');
        res.setHeader('Cache-Control','s-maxage=120, stale-while-revalidate=900');
        res.setHeader('X-RSS-Upstream',url.hostname);
        return res.status(200).send(body);
      }
      try{await response.body?.cancel();}catch{}
      if(![403,406,429,500,502,503,504].includes(response.status)) break;
    }

    return res.status(502).json({
      ok:false,
      error:lastStatus===403?'取得先に一時的に拒否されました。自動再試行または保存済み記事を使用します。':`RSS取得エラー (${lastStatus||'unknown'})`
    });
  } catch(error) {
    return res.status(502).json({ok:false,error:error?.name==='TimeoutError'?'RSS取得がタイムアウトしました':(error?.message||'RSS取得エラー')});
  }
}
...
function twitterVideoHeaders(req,{probe=false}={}) {
  ...
  const headers={
    ...
    'Accept-Encoding':'identity',Referer:'https://x.com/',Origin:'https://x.com'
  };
...
async function fetchTwitterVideo(inputUrl,init={}) {
  ...
  for(let count=0;count<=VIDEO_REDIRECTS;count+=1) {
    ...
    let response;
    try { response=await fetch(current,{...init,redirect:'manual',signal:controller.signal}); }
...
function twitterVideoProxyPath(url) {
  const params=new URLSearchParams({mode:'twitter-video',url:url.href});
...
function rewriteHlsPlaylist(text,baseUrl) {
  ...
  return String(text||'').split(/\r?\n/).map(line=>{
    if(!line) return line;
    if(line.startsWith('#')) return line.replace(/URI="([^"]+)"/gi,(_,raw)=>`URI="${rewriteUri(raw)}"`);
...
async function handleTwitterVideo(req,res) {
  ...
  try {
    const {response:upstream,finalUrl}=await fetchTwitterVideo(target,{method:'GET',headers:twitterVideoHeaders(req,{probe})});
    const contentType=upstream.headers.get('content-type')||'application/octet-stream';
    const ok=upstream.status>=200&&upstream.status<400;
    if(probe) {
      try{await upstream.body?.cancel();}catch{}
      return res.status(ok?200:502).json({ok,upstreamStatus:upstream.status,contentType,range:upstream.headers.get('accept-ranges')||'',finalHost:finalUrl.hostname});
    }
    if(!ok) {
      try{await upstream.body?.cancel();}catch{}
    ...
    if(isHlsPlaylist(contentType,finalUrl)) {
      ...
      res.statusCode=200;res.setHeader('Content-Type','application/vnd.apple.mpegurl; charset=utf-8');res.setHeader('Content-Length',Buffer.byteLength(playlist));
    ...
    if(req.method==='HEAD'||!upstream.body){try{await upstream.body?.cancel();}catch{}return res.end();}
    const body=Readable.fromWeb(upstream.body);
    req.on?.('aborted',()=>body.destroy());
    res.on?.('close',()=>{if(!res.writableEnded) body.destroy();});
    body.on('error',error=>{console.error('[rss twitter-video stream]',error);if(!res.headersSent)res.status(502).end('Twitter video stream failed');else res.destroy?.(error);});
...
export default async function handler(req,res) {
  if(String(first(req.query?.mode)||'').trim()==='twitter-video') return handleTwitterVideo(req,res);
