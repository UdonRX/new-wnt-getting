...
export function readerTable(event, rows = []) {
  ...
  if (readerDebugEnabled() && typeof console !== 'undefined' && typeof console.table === 'function') {
...
export function articleDateDebug(item = {}, index = 0) {
  ...
  const ms = Number.isFinite(Number(item?.publishedTimestamp))
    ? Number(item.publishedTimestamp)
    : new Date(item?.pubDate || 0).getTime();
  const timestamp = Number.isFinite(ms) && ms > 0 ? ms : 0;
  return {
    index,
    id: String(item?.id || ''),
