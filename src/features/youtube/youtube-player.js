...
export function mountYouTubePlayer({ host, queue, index = 0, shorts = false } = {}) {
  ...
  const render = async () => {
    ...
    const panel = el('section', {
      class: `twitch-inline-player youtube-inline-player ${shorts ? 'youtube-shorts-inline' : ''}`
    });
    activePanel = panel;
    ...
    head.append(
      el('div', {}, [
        el('div', {
          class: 'twitch-inline-kicker',
          text: shorts ? 'SHORTS' : (item.kind === 'live' ? 'LIVE' : '動画')
        }),
    ...
    const holderId = `yt-v214-player-${Date.now()}-${current}`;
    ...
    // iframeは移動させず、画面外へ出た時だけタイトルを上部固定する。
    ...
    try {
      ...
      if (panel.isConnected) {
        info.append(el('div', {
          class: 'error-box',
          text: `YouTubeプレイヤーを読み込めませんでした: ${error.message}`
        }));
      }
    }
  };
