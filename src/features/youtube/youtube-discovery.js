...
export async function openYouTubeDiscovery({seedItems=[],registeredChannelIds=[],onRegister,onClose}={}){
  ...
  const rail=document.createElement('div');rail.className='youtube-discovery-rail';const like=document.createElement('button'),add=document.createElement('button'),more=document.createElement('button');
  for(const [button,icon,label] of [[like,'♡','興味あり'],[add,'＋','登録'],[more,'⋮','その他']]){button.type='button';button.className='youtube-discovery-action';button.innerHTML=`<span>${icon}</span><small>${label}</small>`;rail.append(button)}overlay.append(rail);
  ...
  function loadIndex(next,{reason='skip'}={}){if(next<0||next>=pool.length)return false;commit(reason);current=next;lastCommitted='';updateUi();try{discoveryPlayer?.loadVideoById?.({videoId:item().videoId,startSeconds:0});discoveryPlayer?.playVideo?.()}catch{}maybeRefill();return true}
  ...
  async function openGrid(){if(gridOpen||!item().channelId)return;gridOpen=true;try{discoveryPlayer?.pauseVideo?.()}catch{}const x=item(),panel=document.createElement('section');panel.className='youtube-creator-grid';const head=document.createElement('header');head.className='youtube-creator-grid-head';const back=document.createElement('button');back.type='button';back.className='youtube-creator-grid-back';back.textContent='‹';const copy=document.createElement('div');copy.className='youtube-creator-grid-copy';copy.innerHTML=`<strong>${x.channelName||'YouTube'}</strong><small>Shorts</small>`;head.append(back,copy,document.createElement('span'));const body=document.createElement('div');body.className='youtube-creator-grid-body';body.innerHTML='<div class="youtube-creator-grid-loading">Shortsを読み込み中…</div>';panel.append(head,body);overlay.append(panel);
    const returnToVideo=()=>{panel.remove();gridOpen=false;try{discoveryPlayer?.playVideo?.()}catch{}};back.onclick=returnToVideo;let sx=0,sy=0;panel.addEventListener('touchstart',e=>{const t=e.changedTouches?.[0];if(t){sx=t.clientX;sy=t.clientY}},{passive:true});panel.addEventListener('touchend',e=>{const t=e.changedTouches?.[0];if(!t)return;const dx=t.clientX-sx,dy=t.clientY-sy;if(sx<=42&&dx>58&&Math.abs(dx)>Math.abs(dy)*1.15)returnToVideo()},{passive:true});
  ...
  close.onclick=()=>{commit('close');closeDiscovery();onClose?.()};channel.onclick=openGrid;
  ...
  more.onclick=()=>{menu?.remove();menu=document.createElement('div');menu.className='youtube-discovery-menu';const uninterested=document.createElement('button');uninterested.type='button';uninterested.textContent='興味なし';const hide=document.createElement('button');hide.type='button';hide.textContent='このチャンネルを発見に出さない';menu.append(uninterested,hide);overlay.append(menu);uninterested.onclick=()=>{const dislikes=setValues(DISLIKES_KEY);dislikes.add(String(item().videoId));saveSet(DISLIKES_KEY,dislikes);upsertHistory(item(),{delta:-5,reason:'dislike'});menu.remove();menu=null;showToast('興味なしを反映しました');next('dislike')};hide.onclick=()=>{const hidden=setValues(HIDDEN_CHANNELS_KEY);hidden.add(String(item().channelId));saveSet(HIDDEN_CHANNELS_KEY,hidden);upsertHistory(item(),{delta:-4,reason:'hide-channel'});const channelId=String(item().channelId);pool=pool.filter((x,i)=>i<=current||String(x.channelId)!==channelId);menu.remove();menu=null;showToast('このチャンネルを発見から除外します');next('hide-channel')}};
  let sx=0,sy=0;gesture.addEventListener('touchstart',e=>{const t=e.changedTouches?.[0];if(t){sx=t.clientX;sy=t.clientY}},{passive:true});gesture.addEventListener('touchend',e=>{if(gridOpen)return;const t=e.changedTouches?.[0];if(!t)return;const dx=t.clientX-sx,dy=t.clientY-sy,ax=Math.abs(dx),ay=Math.abs(dy);if(ax<18&&ay<18){try{const YT=window.YT,state=discoveryPlayer?.getPlayerState?.();if(state===YT?.PlayerState?.PLAYING)discoveryPlayer.pauseVideo();else discoveryPlayer?.playVideo?.()}catch{}return}if(ax>60&&ax>ay*1.15&&dx<0){openGrid();return}if(ay>58&&ay>ax*1.15){if(dy<0)next('skip');else previous()}},{passive:true});
  updateUi();
  try{const YT=await ensureApi();if(!overlay.isConnected)return null;discoveryPlayer=new YT.Player(holder.id,{videoId:item().videoId,playerVars:{autoplay:1,playsinline:1,rel:0,controls:0,cc_load_policy:0,modestbranding:1,origin:location.origin},events:{onReady:event=>{try{event.target.getIframe?.().setAttribute('referrerpolicy','strict-origin-when-cross-origin');event.target.playVideo()}catch{}},onStateChange:event=>{if(event.data===YT.PlayerState.ENDED){commit('ended');next('ended')}},onError:()=>next('error')}})}catch(error){showToast(error?.message||'Shortsプレイヤーを開始できませんでした')}
  return{close:()=>{closeDiscovery();onClose?.()}};
}
