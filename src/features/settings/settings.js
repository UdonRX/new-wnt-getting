...
export async function renderSettings(root,{navigate}){
  ...
  const appearance=el('div',{class:'card settings-section'});appearance.append(el('h2',{text:'画面の輪郭'}));appearance.append(row('機能ごとの輪郭色',toggle(s.edgeEnabled,v=>patchSettings({edgeEnabled:v})),'PWAの表示領域に細い色を付けます。'));
  ...
  const colors=[['home','ホーム'],['weather','天気'],['news','ニュース'],['knowledge','知識'],['papers','論文'],['youtube','YouTube'],['twitch','Twitch'],['twitter','SNS'],['settings','設定']];colors.forEach(([k,l])=>appearance.append(row(`${l}の色`,color(s.colors[k],v=>patchSettings({colors:{[k]:v}})))));

  const glass=el('div',{class:'card settings-section'});glass.append(el('h2',{text:'下部ナビ'}));glass.append(row('Liquid Glass風',toggle(s.glassEnabled,v=>patchSettings({glassEnabled:v})),'WebではiOSのネイティブ素材を直接呼べないため、Safariのbackdrop-filterで近い質感を再現します。'));glass.append(row('透明度',range(s.glassOpacity,.35,.95,.05,v=>patchSettings({glassOpacity:v}))));glass.append(row('ぼかし',range(s.glassBlur,8,42,1,v=>patchSettings({glassBlur:v}))));
  ...
  screen.append(appearance,glass,behavior,data);root.replaceChildren(screen);
}
