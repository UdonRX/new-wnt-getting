const JMA_CODES = {
  ...
  '新潟県':'150000','富山県':'160000','石川県':'170000','福井県':'180000','山梨県':'190000','長野県':'200000','岐阜県':'210000','静岡県':'220000','愛知県':'230000','三重県':'240000',
  '滋賀県':'250000','京都府':'260000','大阪府':'270000','兵庫県':'280000','奈良県':'290000','和歌山県':'300000','鳥取県':'310000','島根県':'320000','岡山県':'330000','広島県':'340000','山口県':'350000',
  '徳島県':'360000','香川県':'370000','愛媛県':'380000','高知県':'390000','福岡県':'400000','佐賀県':'410000','長崎県':'420000','熊本県':'430000','大分県':'440000','宮崎県':'450000','鹿児島県':'460100'
};
...
export async function geocodeJapan(name) {
  ...
  return (data.results || []).map(r => ({
    ...
    lat: Number(r.latitude), lon: Number(r.longitude), admin1:r.admin1 || '',
    jmaCode: jmaCodeForAdmin(r.admin1 || '', `${r.name || ''} ${r.admin2 || ''}`)
  }));
}
...
export function parseOfficialForecast(data) {
  ...
  if (wxSeries?.areas?.[0]) {
    const area = wxSeries.areas[0];
    (wxSeries.timeDefines || []).forEach((t, i) => out.push({
      date: t.slice(0,10),
  ...
  if (weekly?.areas?.[0]) {
    const area = weekly.areas[0];
    (weekly.timeDefines || []).forEach((t, i) => {
      const date = t.slice(0,10);
      if (out.some(x=>x.date===date)) return;
      out.push({ date, weather:'週間予報', code:area.weatherCodes?.[i] || '', pop:area.pops?.[i] || '', reliability:area.reliabilities?.[i] || '', official:true });
    });
  }
