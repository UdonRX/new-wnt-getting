...
function swipeIndicator(container){if(!container)return null;let indicator=container.querySelector(':scope > .reader-swipe-progress-indicator');if(indicator)return indicator;indicator=document.createElement('span');indicator.className='reader-swipe-progress-indicator';indicator.setAttribute('aria-hidden','true');container.append(indicator);return indicator;}
...
document.addEventListener('touchstart',event=>{
  ...
  const touch=event.touches[0];readerGesture={host,screen:host.closest('.reader-screen'),startX:touch.clientX,startY:touch.clientY,startAt:performance.now(),dx:0,dy:0,horizontal:false,plan:null,visual:host.querySelector('.reader-bento-view')||host.firstElementChild};
...
document.addEventListener('touchmove',event=>{
  ...
  if(!g.horizontal){if(Math.abs(g.dy)>12&&Math.abs(g.dy)>Math.abs(g.dx)*1.08){readerGesture=null;return;}if(Math.abs(g.dx)<7||Math.abs(g.dx)<=Math.abs(g.dy)*1.08)return;g.horizontal=true;g.plan=readerPlan(g.screen,g.dx<0?1:-1);g.visual?.classList.add('reader-horizontal-live');}
  if(event.cancelable)event.preventDefault();
  const travel=readerTravel(g.host.clientWidth);const p=clamp(Math.abs(g.dx)/travel);if(!g.plan)g.plan=readerPlan(g.screen,g.dx<0?1:-1);paintReaderPlan(g.plan,p);if(g.visual)g.visual.style.transform=`translate3d(${(g.dx*.18).toFixed(1)}px,0,0)`;
...
document.addEventListener('touchend',event=>{
  ...
  const elapsed=Math.max(16,performance.now()-g.startAt);const speed=Math.abs(g.dx)/elapsed;const travel=readerTravel(g.host.clientWidth);const p=clamp(Math.abs(g.dx)/travel);const quickFlick=Math.abs(g.dx)>=36&&speed>=.46;const commit=Boolean(g.plan&&(p>=.5||quickFlick));
  if(g.visual){g.visual.style.transition='transform 170ms cubic-bezier(.2,.82,.2,1)';g.visual.style.transform=commit?`translate3d(${g.dx<0?'-42px':'42px'},0,0)`:'translate3d(0,0,0)';setTimeout(()=>{if(g.visual){g.visual.style.transition='';g.visual.style.transform='';g.visual.classList.remove('reader-horizontal-live');}},205);}
  if(g.plan){paintReaderPlan(g.plan,commit?1:0);setTimeout(()=>clearReaderPlan(g.plan),190);}
  if(commit)setTimeout(()=>g.plan?.commit?.(),125);
},{capture:true,passive:false});
...
function installTwitterHeader(){
  const host=document.querySelector('.twitter-feed-host');const screen=host?.closest('.screen');if(!screen)return false;if(screen.dataset.pdv2194Twitter==='1')return true;
  ...
  screen.dataset.pdv2194Twitter='1';const wrapper=document.createElement('div');wrapper.className='twitter-rich-sticky';screen.insertBefore(wrapper,topbar);wrapper.append(topbar,chips);
...
function patchYouTubePlayer(){
  const YT=window.YT;if(!YT?.Player||YT.Player.__pdv2194Resume)return false;const Original=YT.Player;
  ...
  WrappedPlayer.prototype=Original.prototype;Object.assign(WrappedPlayer,Original);WrappedPlayer.__pdv2194Resume=true;YT.Player=WrappedPlayer;return true;
}
...
const youtubeLandscapeState=new WeakMap();
const twitchLandscapeState=new WeakMap();
...
function enterYoutubeLandscape(panel){if(!panel?.classList.contains('youtube-css-landscape'))return;let state=youtubeLandscapeState.get(panel);if(state){showLandscapeUi(panel,state);return;}const viewport=panel.querySelector('.youtube-landscape-viewport');const controls=panel.querySelector('.youtube-landscape-controls');if(!viewport||!controls)return;const catcher=document.createElement('button');catcher.type='button';catcher.className='media-landscape-tap-catcher';catcher.setAttribute('aria-label','操作ボタンを表示');viewport.append(catcher);state={catcher,controls,timer:null};youtubeLandscapeState.set(panel,state);catcher.addEventListener('click',()=>showLandscapeUi(panel,state));controls.addEventListener('pointerdown',()=>showLandscapeUi(panel,state),{passive:true});controls.addEventListener('click',()=>showLandscapeUi(panel,state));showLandscapeUi(panel,state);}
function exitYoutubeLandscape(panel){const state=youtubeLandscapeState.get(panel);if(!state)return;if(state.timer)clearTimeout(state.timer);panel.classList.remove('pdv2-landscape-ui-visible');state.catcher.remove();youtubeLandscapeState.delete(panel);}
function syncYoutubeLandscape(panel){if(!panel?.isConnected)return;if(panel.classList.contains('youtube-css-landscape'))enterYoutubeLandscape(panel);else exitYoutubeLandscape(panel);}
function enterTwitchLandscape(panel){if(!panel?.classList.contains('twitch-css-landscape'))return;let state=twitchLandscapeState.get(panel);if(state){showLandscapeUi(panel,state);return;}const controls=panel.querySelector('.twitch-inline-controls');if(!controls)return;const home=controls.parentNode;const next=controls.nextSibling;const overlay=document.createElement('div');overlay.className='twitch-landscape-overlay-controls';const catcher=document.createElement('button');catcher.type='button';catcher.className='media-landscape-tap-catcher twitch-landscape-tap-catcher';catcher.setAttribute('aria-label','操作ボタンを表示');panel.append(catcher,overlay);overlay.append(controls);state={catcher,overlay,controls,home,next,timer:null};twitchLandscapeState.set(panel,state);catcher.addEventListener('click',()=>showLandscapeUi(panel,state));overlay.addEventListener('pointerdown',()=>showLandscapeUi(panel,state),{passive:true});overlay.addEventListener('click',()=>showLandscapeUi(panel,state));showLandscapeUi(panel,state);}
function exitTwitchLandscape(panel){const state=twitchLandscapeState.get(panel);if(!state)return;if(state.timer)clearTimeout(state.timer);panel.classList.remove('pdv2-landscape-ui-visible');if(state.home?.isConnected){if(state.next&&state.next.parentNode===state.home)state.home.insertBefore(state.controls,state.next);else state.home.append(state.controls);}state.catcher.remove();state.overlay.remove();twitchLandscapeState.delete(panel);}
function syncTwitchLandscape(panel){if(!panel?.isConnected)return;if(panel.classList.contains('twitch-css-landscape'))enterTwitchLandscape(panel);else exitTwitchLandscape(panel);}

document.addEventListener('click',event=>{
  const youtubeButton=event.target?.closest?.('.youtube-inline-player .youtube-orientation-btn');if(youtubeButton){const panel=youtubeButton.closest('.youtube-inline-player');setTimeout(()=>syncYoutubeLandscape(panel),0);return;}
  const twitchButton=event.target?.closest?.('.twitch-inline-player:not(.youtube-inline-player) .twitch-orientation-btn, .twitch-inline-player:not(.youtube-inline-player) .media-portrait-return-btn');if(twitchButton){const panel=twitchButton.closest('.twitch-inline-player');setTimeout(()=>syncTwitchLandscape(panel),0);}
});
window.addEventListener('pdv2:before-navigate',()=>{document.querySelectorAll('.youtube-inline-player').forEach(exitYoutubeLandscape);document.querySelectorAll('.twitch-inline-player:not(.youtube-inline-player)').forEach(exitTwitchLandscape);readerFab?.remove();readerFab=null;readerFabSource=null;readerFabScreen=null;});

function twitchThumbBad(img){const src=String(img?.currentSrc||img?.src||'');return img?.classList?.contains('thumb-missing')||/previews-ttv\/live_user_|404[_-](?:preview|processing)|processing[_-]?preview|placeholder|question/i.test(src);}
function twitchProfileMap(){const map=new Map();try{const cached=JSON.parse(localStorage.getItem('pdv2:twitchCache:v2170')||'null');for(const snapshot of cached?.rows||[]){const profile=String(snapshot?.broadcaster?.profileImageUrl||'');if(!profile)continue;for(const video of snapshot?.archives||[]){const title=String(video?.title||'').trim();if(title&&!map.has(title))map.set(title,profile);}}}catch{}return map;}
function repairTwitchThumbnails(){
  const profiles=twitchProfileMap();document.querySelectorAll('.media-list').forEach(list=>{const images=[...list.querySelectorAll('img.twitch-archive-thumb')];if(!images.length)return;const donors=images.filter(img=>img.complete&&img.naturalWidth>80&&!twitchThumbBad(img));images.forEach(img=>{if(!twitchThumbBad(img))return;const row=img.closest('.media-row');const title=String(row?.querySelector('.media-title')?.textContent||'').trim();const profile=profiles.get(title)||'';const donor=donors.find(candidate=>candidate!==img);const src=profile||String(donor?.currentSrc||donor?.src||'');if(!src||img.dataset.pdv2194Thumb===src)return;img.dataset.pdv2194Thumb=src;img.classList.remove('thumb-missing');img.src=src;});});
}

let enhancementRaf=0;
function scheduleEnhancements(){if(enhancementRaf)return;enhancementRaf=requestAnimationFrame(()=>{enhancementRaf=0;ensureReaderFab();if(!installTwitterHeader()&&twitterCleanup){twitterCleanup();twitterCleanup=null;}repairTwitchThumbnails();});}
const enhancementObserver=new MutationObserver(scheduleEnhancements);enhancementObserver.observe(document.documentElement,{subtree:true,childList:true});window.addEventListener('load',scheduleEnhancements,{once:true});requestAnimationFrame(scheduleEnhancements);
