...
function extractDataP(html = '', rawUrl = '') {
  const source = String(html);
  const legacy = source.match(/<c-wiz\b[^>]*\bdata-p\s*=\s*(["'])([\s\S]*?)\1/i);
  if (legacy?.[2]) return decodeHtml(legacy[2]);

  const readAttr = name => {
    const match = source.match(new RegExp(`\\b${name}\\s*=\\s*(["'])([\\s\\S]*?)\\1`, 'i'));
    return match?.[2] ? decodeHtml(match[2]) : '';
  };
  const signature = readAttr('data-n-a-sg');
  const timestampRaw = readAttr('data-n-a-ts');
  const id = readAttr('data-n-a-id') || googleNewsArticleId(rawUrl);
  const timestamp = Number(timestampRaw);
  if (!signature || !id || !Number.isFinite(timestamp) || timestamp <= 0) return '';

  const context = [
    ['X', 'X', ['X', 'X'], null, null, 1, 1, 'US:en', null, 1, null, null, null, null, null, 0, 1],
    'X', 'X', 1, [1, 1, 1], 1, 1, null, 0, 0, null, 0
  ];
  // Shape this as the old data-p payload so the existing Fbv4je POST path stays shared.
  return JSON.stringify(['garturlreq', context, id, null, null, null, null, timestamp, signature]);
}

async function postGoogleNewsBatchexecute(dataP, refererUrl, timeoutMs) {
  const source = String(dataP || '').trim();
  if (!source) throw new Error('missing-google-news-decode-params');
  let requestObject;
  try {
...
async function resolveGoogleNewsPublisherUrlLight(rawUrl, timeoutMs) {
  ...
  const decodeUrl = articleId ? `https://news.google.com/articles/${encodeURIComponent(articleId)}` : original;
  const page = await fetchHtmlPrefix(decodeUrl, { timeoutMs, maxBytes: MAX_HTML_PREFIX_BYTES });
  ...
  const dataP = extractDataP(page.text, decodeUrl);
