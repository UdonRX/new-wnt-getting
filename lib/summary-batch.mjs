...
function firstChars(value = '', limit = INPUT_LIMIT) {
...
function researchSummaryFromBody(body = {}) {
  ...
  const cardText = (value, max = 64) => {
    const text = clean(value, max * 4).replace(/[\s、,;；:：]+$/g, '').trim();
    const chars = Array.from(text);
    if (!chars.length) return '';
    if (chars.length < max) return /[。！？!?]$/.test(text) ? text : `${text}。`;
    return `${chars.slice(0, max - 1).join('').replace(/[\s、,;；:：]+$/g, '')}。`;
  };
  const conclusionText = cardText(overview);
  const backgroundText = cardText(`${organization}が公開した、${category}に関する技術・改善事例です`);
  const applicationText = cardText(application);
  return {
    headline: clean(body.title, 100) || '技術リサーチ',
    lines: [
...
function cacheKey(item = {}) {
  const url = clean(item.url || item.link, 1800);
  const title = clean(item.title, 500);
  const mode = clean(item.mode, 48) || 'auto';
...
async function prepareItem(raw = {}) {
  ...
  const description = clean(raw.description, 2400);
  const url = clean(raw.url || raw.link, 2400);
  const base = {
    url,
    title,
    source: clean(raw.source, 160),
    category: clean(raw.category, 160),
    mode: clean(raw.mode, 48) || 'auto'
  };

  const preparedResearch = researchSummaryFromBody(raw);
  if (preparedResearch) {
    return {
      ...base,
      text: firstChars(description),
      preparedSource: 'web-research',
      preparedSummary: preparedResearch
    };
  }

  // The batch path intentionally prefers the real publisher/article/PDF over
  // RSS snippets. article-reader also follows citation/download PDF links.
  if (url) {
    try {
      const article = await Promise.race([
        extractArticleFromUrl(url, { maxTextLength: 2200, preferPdf: true }),
        new Promise((_, reject) => setTimeout(() => reject(new Error('batch article timeout')), 6500))
      ]);
      ...
      if (text.length >= 70 && !GENERIC_RE.test(text)) {
        return {
          ...
          text,
          preparedSource: article?.sourceType === 'pdf' ? 'pdf' : 'article'
        };
      }
    } catch (error) {
      console.warn('[summary-batch] article fallback to RSS', error?.message || error);
    }
  }

  if (descriptionLooksReal(title, description)) {
    return { ...base, text: firstChars(description), preparedSource: 'rss' };
  }

  const fallback = description.length >= 45 && !GENERIC_RE.test(description) ? firstChars(description) : '';
  return { ...base, text: fallback, preparedSource: fallback ? 'rss-short' : 'missing' };
}
...
function buildSummary(row = {}, prepared = {}, model = '') {
  const lines = [row.conclusion, row.background, row.impact].map(normalizeSentence);
  if (lines.some(line => Array.from(line).length < 12)) return null;
  if (lines.some(mostlyEnglish)) return null;
  if (new Set(lines.map(line => line.replace(/[\s。、，,.!！?？]/g, '').toLowerCase())).size !== 3) return null;
  return {
    ...
    contentSource: prepared.preparedSource,
    cacheable: true,
    fastPath: 'summary-batch-10x1'
  };
}
  ...
  properties: {
    summaries: {
      ...
      items: {
        ...
        required: ['index', 'headline', 'conclusion', 'background', 'impact']
      }
    }
  },
  required: ['summaries']
...
function promptFor(items) {
  const blocks = items.map(({ batchIndex, prepared }) => [
    `--- ARTICLE ${batchIndex} ---`,
    `タイトル: ${prepared.title || '不明'}`,
    prepared.source ? `媒体: ${prepared.source}` : '',
    prepared.category ? `カテゴリ: ${prepared.category}` : '',
    '記事冒頭:',
    prepared.text
  ].filter(Boolean).join('\n'));

  return [
    '以下の複数記事を、それぞれ独立に日本語で要約してください。',
    '各記事は与えられた本文だけを根拠にし、別の記事の情報を混ぜないでください。',
    'headlineは18〜42文字。conclusion/background/impactは各16〜80文字の1文で、必ず句点で完結させてください。',
    '結論/事実には最重要の具体的事実、背景/特徴には背景・方法・特徴、影響/展望には本文から読み取れる意味・影響・今後を入れてください。',
    '本文にない推測、一般論、補完は禁止です。indexはARTICLE番号をそのまま返してください。',
    '',
    ...blocks
  ].join('\n');
}
...
async function generateBatch(items) {
  ...
  for (const model of modelList()) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error?.message || `Gemini batch ${response.status}`);
...
export default async function summaryBatch(req, res) {
  res.setHeader('Cache-Control', 'no-store');
  const raw = typeof req.body === 'string'
    ? (() => { try { return JSON.parse(req.body); } catch { return {}; } })()
    : (req.body || {});
  const incoming = Array.isArray(raw.items) ? raw.items.slice(0, MAX_ITEMS) : [];
  if (!incoming.length) return res.status(200).json({ results: [], batch: 'summary-batch', generated: 0, cached: 0 });
  ...
  prepared.forEach((item, index) => {
    ...
    if (item.preparedSummary) {
      putCache(key, item.preparedSummary);
      results[index] = { index, url: item.url, mode: item.mode, summary: item.preparedSummary, preparedSource: item.preparedSource };
      return;
    }
    const hit = memoryCache.get(key);
    if (hit) {
      results[index] = { index, url: item.url, mode: item.mode, summary: hit, preparedSource: item.preparedSource };
      cachedCount += 1;
      return;
    }
  ...
  if (missing.length) {
    try {
      const { rows, model } = await generateBatch(missing);
      const byIndex = new Map(rows.map(row => [Number(row?.index), row]));
      for (const entry of missing) {
        const row = byIndex.get(entry.batchIndex);
        if (!row) continue;
        const summary = buildSummary(row, entry.prepared, model);
        if (!summary) continue;
        putCache(entry.key, summary);
        results[entry.batchIndex] = {
          index: entry.batchIndex,
          url: entry.prepared.url,
          mode: entry.prepared.mode,
          summary,
          preparedSource: entry.prepared.preparedSource
        };
        generated += 1;
      }
    } catch (error) {
      console.warn('[summary-batch] generate failed', error?.message || error);
    }
  }
  ...
  prepared.forEach((item, index) => {
    ...
    results[index] = {
      ...
      mode: item.mode,
      summary: null,
      preparedSource: item.preparedSource
    };
  });

  return res.status(200).json({ results, batch: 'summary-batch', generated, cached: cachedCount });
}
