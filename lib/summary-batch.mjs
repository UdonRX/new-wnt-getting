...
function researchSummaryFromBody(body = {}) {
  ...
  const target = sentence(`${organization}を対象にした情報です`);
  const categoryOverview = sentence(`［${category}］${overview}`);
  const applicationText = sentence(application);
  return {
    headline: clean(body.title, 100) || '技術リサーチ',
    lines: [
      { label: '対象企業/組織名', text: target },
      { label: 'カテゴリ・概要', text: categoryOverview },
      { label: '応用着眼点', text: applicationText }
    ],
    short: target,
    points: [categoryOverview, applicationText],
