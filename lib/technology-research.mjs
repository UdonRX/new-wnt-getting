...
function canonicalSourceType(value = '') {
  ...
  if (/研究機関|大学|公的研究|scienceportal|researcher|researchpr|学術広報/.test(text)) return '研究機関PR';
...
function canonicalCategory(value = '', raw = {}) {
  ...
  if (/異分野|アイデア|着想|応用ネタ|転用|cross.?domain|innovation/i.test(haystack)) return '異分野アイデア';
...
function normalizeItems(rawItems, rawResponse = {}) {
  ...
  for (const raw of Array.isArray(rawItems) ? rawItems : []) {
    ...
    const organization = clean(raw?.organization || raw?.company || raw?.brand || raw?.institution, 160);
...
function mergeItems(...groups) {
  const raw = groups.flat().map(item => ({
    ...item,
    publishedAt: item?.pubDate instanceof Date ? item.pubDate.toISOString().slice(0, 10) : item?.publishedAt,
    url: item?.url
  }));
...
6. 「概要」と「応用着眼点」は各80〜180文字程度。概要は元ページの事実、応用着眼点は技術スカウトとしての示唆を分ける。
7. sourceType と category は下記JSONの候補文字列を完全一致で使用する。
...
function toRssItems(items) {
  return items.map(item => ({
    ...
    sourceName: `技術リサーチ｜${item.sourceType}｜${item.category}`,
    image: item.image,
    description: [
      '技術リサーチ: Web調査済み',
...
async function research({ refresh = false } = {}) {
  ...
  if (missingTypes.length) {
    try {
      ...
      items = mergeItems(items, supplement);
    } catch (error) {
      console.warn('[technology-research] supplement failed', error?.message || error);
    }
  }

  if (!items.length) throw new Error('技術リサーチの有効候補を1件も取得できませんでした');

  const xml = rssXml(
    '技術リサーチ',
    `5情報源×各2件を目標にした最新技術・競合・異分野アイデア（今回 ${items.length}/10件）`,
    toRssItems(items)
  );
