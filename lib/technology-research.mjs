...
function webAccept(raw,category){const title=clean(raw.title,320),url=safeUrl(raw.url),summary=clean(raw.content||raw.description,1400),text=`${title} ${summary}`,isNote=host(url)==='note.com';if(!title||!url||!summary||blocked(url)||PROMO_RE.test(text)||LOW_VALUE_WEB_RE.test(text))return null;let score=Math.round((Number(raw.score)||0)*28);if(TOPIC_RE.test(text))score+=24;if(PRACTICAL_RE.test(text))score+=8;if(category==='生技基礎'&&FOUNDATION_RE.test(text))score+=24;if(category==='改善事例'&&CASE_RE.test(text))score+=24;if(category==='技術革新'&&INNOVATION_RE.test(text))score+=24;if(category==='製品・製造技術'&&PRODUCT_RE.test(text))score+=24;if(category==='異業種横展開'&&CROSS_RE.test(text)&&CASE_RE.test(text))score+=28;if(isNote)score+=4;if(score<36)return null;return item({category,title,url,summary,source:isNote?'note':host(url),organization:isNote?'note':host(url),date:raw.published_date||'',image:raw.image,acquisition:isNote?'Tavily note.com専用検索＋機械採点':'Tavily一般Web検索＋機械採点',score:Math.min(100,score),reason:`カテゴリ語・実務語・検索関連度から採用（score ${Math.min(100,score)}）`})}
