...
const PREFERRED_CASE_RE=/(改善|カイゼン|問題解決|トラブル|保全|点検|故障|寿命|ムダ|ロス|タクト|段取り|治具|設備|品質|不良|歩留まり|ライン立上げ|ライン立ち上げ|初期流動|からくり|自動化|省人|工数|生産性|稼働|停止|予防|予知|原価|コスト|TPM|自主保全|可動率)/i;
const PREFERRED_SKIP_RE=/(キャリア|転職|就活|面接|年収|求人|職種図鑑|メンバーシップ|案内板|有料マガジン|販売中)/i;
...
const PREFERRED_CASE_FEEDS=[
 {name:'MKM_tech',url:'https://note.com/alert_hyssop1862/rss'},
 {name:'生技ちゃん',url:'https://note.com/u1_38/rss'},
 {name:'生技の父',url:'https://note.com/juicy_roses9363/rss'},
 {name:'Yuno Tech Note',url:'https://note.com/yuno_lab/rss'},
 {name:'PE Digital',url:'https://www.pe-digital.jp/feed/'},
 {name:'PE Digital｜からくり改善',url:'https://www.pe-digital.jp/atags/%E3%81%8B%E3%82%89%E3%81%8F%E3%82%8A%E6%94%B9%E5%96%84/feed/'}
];
const PREFERRED_CASE_PAGES=[
 {name:'PE Digital',url:'https://www.pe-digital.jp/'},
 {name:'PE Digital｜からくり改善',url:'https://www.pe-digital.jp/atags/%E3%81%8B%E3%82%89%E3%81%8F%E3%82%8A%E6%94%B9%E5%96%84/'}
];
...
function preferredCaseOk(title='',summary='',url=''){const text=`${title} ${summary}`;if(!title||!url||blocked(url)||eventGuideLike(title,summary,url)||PROMO_RE.test(text)||LOW_VALUE_WEB_RE.test(text)||PREFERRED_SKIP_RE.test(text))return false;return PREFERRED_CASE_RE.test(text)&&(TOPIC_RE.test(text)||/(からくり|TPM|自主保全|問題解決|設備管理|保全)/i.test(text))}

async function jstage(term){const u=new URL('https://api.jstage.jst.go.jp/searchapi/do');u.searchParams.set('service','3');u.searchParams.set('text',term);u.searchParams.set('count','40');const r=await fetch(u,{headers:{Accept:'application/atom+xml,application/xml,text/xml','User-Agent':'PersonalDashboardTechnologyResearch/11.0'},signal:AbortSignal.timeout(12000)});if(!r.ok)throw new Error(`J-STAGE HTTP ${r.status}`);const dom=xmlDom(await r.text());try{return [...dom.window.document.querySelectorAll('entry')].map(e=>{const title=txt(e,['article_title > ja','article_title > en','title']);const link=txt(e,['article_link > ja','article_link > en'])||e.querySelector('link')?.getAttribute('href')||txt(e,['id']);const summary=txt(e,['abstract > ja','abstract > en','summary','description'])||`掲載誌: ${txt(e,['material_title > ja','material_title > en'])}`;const full=txt(e,['pubdate','publication_date','online_pubdate','published']);const year=txt(e,['pubyear']);return item({category:'論文・研究',title,url:link,summary,source:'J-STAGE',organization:'J-STAGE',date:full||year,acquisition:`J-STAGE WebAPI（${term}）`,score:82,reason:'論文検索テーマに合致'})}).filter(Boolean)}finally{dom.window.close()}}
...
async function preferredCaseFeed(source){const rows=await fetchRssSource({name:source.name,url:source.url,maxItems:60});return rows.map(x=>{const title=clean(x.title,320),summary=clean(x.description||x.title,1400),url=safeUrl(x.link);if(!preferredCaseOk(title,summary,url))return null;return item({category:'改善事例',title,url,summary,source:source.name,organization:x.author||source.name,date:x.pubDate,image:x.image,acquisition:`指定優先媒体RSS（${source.name}）`,score:96,reason:'ユーザー指定の改善事例優先媒体から採用'})}).filter(Boolean)}
async function preferredCasePage(source){const r=await fetch(source.url,{headers:{Accept:'text/html,application/xhtml+xml','User-Agent':'PersonalDashboardTechnologyResearch/11.0'},signal:AbortSignal.timeout(10000)});if(!r.ok)throw new Error(`${source.name} HTTP ${r.status}`);const html=await r.text();const vc=new VirtualConsole();const dom=new JSDOM(html,{url:source.url,virtualConsole:vc});try{const out=[];for(const a of dom.window.document.querySelectorAll('a[href]')){const href=a.getAttribute('href')||'';let url='';try{url=new URL(href,source.url).href}catch{continue}if(host(url)!=='pe-digital.jp'||!new URL(url).pathname.includes('/posts/'))continue;const title=clean(a.textContent,320);if(title.length<8)continue;const box=a.closest('article,li,section,div');const summary=clean(box?.textContent||title,1400);if(!preferredCaseOk(title,summary,url))continue;const date=(summary.match(/20\d{2}[.\/-]\d{1,2}[.\/-]\d{1,2}/)||[])[0]||'';const row=item({category:'改善事例',title,url,summary,source:source.name,organization:'日本プラントメンテナンス協会',date,acquisition:`指定優先媒体ページ直接取得（${source.name}）`,score:96,reason:'ユーザー指定の改善事例優先媒体から採用'});if(row)out.push(row)}return dedupe(out)}finally{dom.window.close()}}
async function preferredCases(){const jobs=[...PREFERRED_CASE_FEEDS.map(source=>preferredCaseFeed(source)),...PREFERRED_CASE_PAGES.map(source=>preferredCasePage(source))];const settled=await Promise.allSettled(jobs),items=[],errors=[];for(const result of settled){if(result.status==='fulfilled')items.push(...result.value);else errors.push(clean(result.reason?.message||result.reason,180))}const rows=dedupe(items);console.info('[technology-research][preferred-cases]',{count:rows.length,errors:errors.length,sources:PREFERRED_CASE_FEEDS.length+PREFERRED_CASE_PAGES.length});return{items:rows,errors}}
...
async function research(){if(memoryCache.xml&&Date.now()-memoryCache.at<CACHE_TTL)return memoryCache;const [p,m,w,d]=await Promise.all([papers(),monoist(),webResearch(),preferredCases()]);const selected=sorted([...seedItems(),...p.items,...m.items,...w.items,...d.items]);if(!selected.length)throw new Error('条件を満たす技術リサーチ記事を取得できませんでした');const categoryCounts=Object.fromEntries(CATEGORIES.map(c=>[c,selected.filter(x=>x.category===c).length]));const xml=rssXml('技術リサーチ',`24時間キャッシュ＋Tavily一般Web/note検索＋J-STAGE/CiNii/MONOist＋指定改善事例媒体。条件合格記事を全件配信。今回${selected.length}件。`,toRss(selected));const meta={categoryCounts,paperErrors:p.errors.length,monoistErrors:m.errors.length,webErrors:w.errors.length,preferredCaseErrors:d.errors.length,preferredCaseCount:d.items.length,webStats:w.stats,tavilyConfigured:w.configured,tavilyRequestsPlanned:w.requestsPlanned,tavilyRequestsSucceeded:w.requestsSucceeded};Object.assign(memoryCache,{at:Date.now(),xml,items:selected,meta});console.info('[technology-research] final',{count:selected.length,...meta,tavilyDaily:`${daily.used}/${DAILY_LIMIT}`});return memoryCache}
