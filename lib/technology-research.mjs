...
function relevanceScore(item) {
  const hay = `${item.title} ${item.summary} ${item.organization}`;
  return (CORE_RE.test(hay) ? 8 : 0) + (IDEA_RE.test(hay) ? 4 : 0) + ageScore(item);
}

function selectTen(items = []) {
  const normalized = dedupe(items).sort((a, b) => relevanceScore(b) - relevanceScore(a) || normalizeDate(b.pubDate) - normalizeDate(a.pubDate));
  const result = [];
...
async function research({ refresh = false } = {}) {
  ...
  const directItems = dedupe((collected.candidates || [])
    .map(raw => normalizeItem(raw, { direct: true }))
    .filter(Boolean)
    .filter(relevantDirectItem));
  const slot = Math.floor(Date.now() / (4 * 60 * 60 * 1000));
  const rotation = ROTATION_FOCUS[slot % ROTATION_FOCUS.length];

  // まず直接取得済み候補を土台にする。これによりGeminiが5件しか返さなくても、
  // 取得済みRSS/API候補が捨てられて5件固定になることを防ぐ。
...
export async function technologyResearchFeed(req, res) {
  ...
  try {
    ...
    res.setHeader('X-Technology-Research-Type-Counts', JSON.stringify(result.meta?.typeCounts || {}));
