...
const PAYWALL_STRONG_RE = /(?:有料(?:会員|記事)(?:限定|のみ)?|会員限定|有料会員限定|続きを読むには|続き(?:を読む|は).*?(?:ログイン|会員|登録)|ログイン(?:して|が必要|してください)|購読(?:して|が必要)|login\s*required|members?\s*only|premium\s*content)/i;
const PAYWALL_WEAK_RE = /(?:会員登録|サブスクリプション|subscribe\b|subscription\b|sign\s*in\b)/i;
const SYSTEM_STRONG_RE = /(?:\b404\b|\b403\b|\b500\b|\b503\b|not\s*found|page\s*not\s*found|ページが見つかりません|お探しのページ|access\s*denied|forbidden|service\s*unavailable|internal\s*server\s*error)/i;
