...
const PAYWALL_STRONG_RE = /(?:有料会員限定|会員限定(?:記事|コンテンツ)?|有料記事(?:です|となります|のため)|続きを読むには|続き(?:を読む|は).*?(?:ログイン|会員|登録)|ログイン(?:して|が必要|してください)|購読(?:して|が必要)|login\s*required|members?\s*only|premium\s*content)/i;
const PAYWALL_WEAK_RE = /(?:有料会員|有料記事|会員登録|サブスクリプション|subscribe\b|subscription\b|sign\s*in\b)/i;
const SYSTEM_STRONG_RE = /(?:\b404\b|\b403\s+forbidden\b|\b500\s+(?:internal\s+server\s+error|server\s+error)\b|\b503\s+(?:service\s+unavailable)?\b|not\s*found|page\s*not\s*found|ページが見つかりません|お探しのページ|access\s*denied|forbidden|service\s*unavailable|internal\s*server\s*error)/i;
const SYSTEM_WEAK_RE = /(?:javascript\s*(?:error|required|disabled)|エラーが発生|読み込みに失敗|アクセスできません|temporarily\s*unavailable)/i;
...
export function inspectSummaryText(text = '', context = {}) {
  ...
  const sentenceTotal = sentenceCount(cleaned);
  const strongPaywall = PAYWALL_STRONG_RE.test(cleaned);
  const weakPaywall = PAYWALL_WEAK_RE.test(cleaned) && chars < 420 && sentenceTotal < 3;

  if (strongPaywall || weakPaywall) {
    return diagnostic({
      ...base,
      code: 'PAYWALL_AUTH',
  ...
  const strongSystem = SYSTEM_STRONG_RE.test(cleaned) && chars < 900;
  const weakSystem = SYSTEM_WEAK_RE.test(cleaned) && chars < 360 && sentenceTotal < 2;
  if (strongSystem || weakSystem) {
    return diagnostic({
      ...base,
      code: 'SYSTEM_MESSAGE',
  ...
  if (navHits.length >= 3 && sentenceTotal < 2 && chars < 900) {
    return diagnostic({
      ...base,
