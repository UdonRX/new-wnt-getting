...
function buildSummaryObject(row = {}, body = {}, model = '', { fastRequest = false } = {}) {
  const lines = fastRequest
    ? [row.c, row.b, row.i].map(normalizeSentence)
    : [row.conclusion, row.background, row.impact].map(normalizeSentence);
  if (fastRequest && lines.some(line => sentenceContentLength(line) < 20 || sentenceContentLength(line) > 55)) {
    throw new Error('高速AI要約は各20〜55文字である必要があります');
  }
  if (!fastRequest && lines.some(line => Array.from(line).length < 12)) throw new Error('AI要約が短すぎます');
...
function singlePrompt(body = {}, { fastRequest = false, inputLimit = SINGLE_INPUT_LIMIT } = {}) {
  return [
    ...
    fastRequest
      ? 'hは16〜32文字。c/b/iは各20〜55文字の1文で、通常は25〜45文字とし、必ず句点で完結させてください。'
...
export async function generateStructuredSingle(body = {}) {
  ...
  for (let modelIndex = 0; modelIndex < models.length; modelIndex += 1) {
    ...
    try {
      ...
      // fastなnews/knowledgeは2.8秒で打ち切り、別モデル再試行で表示を長引かせない。
      // papersなど非fastだけ従来どおりフォールバックモデルへ進む。
    } finally {
      clearTimeout(timer);
