...
function stripStyleBlocksFromBuffer(input) {
  const buffer = Buffer.isBuffer(input) ? input : Buffer.from(input || '');
  if (!buffer.length) return buffer;

  // HTMLタグはASCIIなのでlatin1で1byte=1文字として位置だけ探索し、
  // 本文の元バイト列はそのまま保持する。これでShift_JIS等でも本文を再エンコードしない。
  ...
  while (cursor < buffer.length) {
    ...
    if (closeStart < 0) {
      parts.push(buffer.subarray(cursor, start));
      removed += 1;
      break;
    }
    const closeEnd = lower.indexOf('>', closeStart + 7);
    if (closeEnd < 0) {
      parts.push(buffer.subarray(cursor, start));
      removed += 1;
      break;
    }
    if (start > cursor) parts.push(buffer.subarray(cursor, start));
    parts.push(Buffer.from(' '));
    removed += 1;
    cursor = closeEnd + 1;
  }

  return removed ? Buffer.concat(parts) : buffer;
}
...
async function resolveGoogleNewsPublisherUrl(rawUrl) {
  ...
  const dom = new JSDOM(stripStyleBlocksFromBuffer(googlePage.buffer), { url: googlePage.finalUrl, contentType: 'text/html' });
  try {
...
function parseArticle(htmlBuffer, pageUrl, maxTextLength) {
  const dom = new JSDOM(stripStyleBlocksFromBuffer(htmlBuffer), { url: pageUrl, contentType: 'text/html' });
    ...
    if (pdfUrl) {
      try {
        ...
        if (pdfResource.kind === 'pdf') {
          ...
          const htmlTitle = (() => {
            try {
              const dom = new JSDOM(stripStyleBlocksFromBuffer(resource.buffer), { url: resource.finalUrl, contentType: 'text/html' });
