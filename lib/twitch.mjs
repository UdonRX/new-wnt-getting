...
function getCredentials(){
  ...
  if(!clientId||!clientSecret){
    throw Object.assign(new Error('TWITCH_CLIENT_ID / TWITCH_CLIENT_SECRET がVercelに設定されていません。'),{statusCode:500,code:'TWITCH_CONFIG'});
  }
...
function cachedSnapshot(login,{freshOnly=false}={}){
  const row=snapshotCache.get(login);
  if(!row?.snapshot)return null;
  const age=Date.now()-Number(row.at||0);
  if(age>(freshOnly?SNAPSHOT_FRESH_MS:SNAPSHOT_STALE_MS)){
    if(!freshOnly)snapshotCache.delete(login);
    return null;
  }
  return {...row.snapshot,_serverCache:freshOnly?'fresh':'stale'};
}
function putSnapshot(login,snapshot){
  snapshotCache.set(login,{at:Date.now(),snapshot});
  while(snapshotCache.size>80)snapshotCache.delete(snapshotCache.keys().next().value);
  return snapshot;
}
  ...
  for(let attempt=0;attempt<3;attempt+=1){
    try{
      ...
      if(!response.ok||!data.access_token){
        throw Object.assign(new Error(data?.message||`Twitch App Access Tokenの取得に失敗しました (HTTP ${response.status})`),{statusCode:response.status||502});
      }
...
async function helix(path,params={}){
  ...
  for(let attempt=0;attempt<4;attempt+=1){
    let credentials;
    try{credentials=await getAppAccessToken({force:forceToken});}
    catch(error){lastError=error;if(attempt<3){await sleep(250*(attempt+1));forceToken=true;continue;}throw error;}
    ...
    try{
      ...
      if([408,425,429,500,502,503,504].includes(response.status)&&attempt<3){
        const retryAfter=Math.min(1800,Math.max(180,Number(response.headers.get('retry-after')||0)*1000||0));
        await sleep(retryAfter||250*(attempt+1));
        continue;
      }
      throw error;
    }catch(error){
      lastError=error;
      if((error?.name==='TimeoutError'||error?.name==='AbortError'||error instanceof TypeError)&&attempt<3){
...
export async function getTwitchChannelSnapshot(input,{archiveLimit=20,force=false,allowStale=true}={}){
  ...
  if(!force){
    const fresh=cachedSnapshot(login,{freshOnly:true});
  ...
  try{
    ...
    return putSnapshot(login,snapshot);
  }catch(error){
    const stale=allowStale?cachedSnapshot(login):null;
    if(stale&&Number(error?.statusCode)!==400&&Number(error?.statusCode)!==404){
      return {...stale,_stale:true,_staleReason:String(error?.message||error||'')};
    }
    throw error;
  }
}
