...
async function prepareItem(raw = {}) {
  ...
  if (url) {
    try {
      ...
      console.warn('[summary-batch-v2188] article prepare failed', error?.message || error);
    }
  }
...
function buildSummary(row = {}, prepared = {}) {
  ...
  if (lines.some(line => Array.from(line).length < 12) || lines.some(mostlyEnglish)) return null;
...
function promptFor(items) {
  ...
  return [
    ...
    '推測、一般論、本文にない補完は禁止です。indexはARTICLE番号をそのまま返してください。',
    '',
    ...blocks
  ].join('\n');
}
...
export default async function summaryBatchV2188(req, res) {
  ...
  if (!incoming.length) return res.status(200).json({ results: [], batch: 'v2188', generated: 0, cached: 0 });
  ...
  prepared.forEach((item, index) => {
    ...
    if (hit) {
      results[index] = { index, url: item.url, mode: item.mode, summary: { ...hit, cache: 'batch-memory-v2188' } };
      cachedCount += 1;
      return;
    }
  ...
  if (missing.length) {
    try {
      ...
      for (const entry of missing) {
        ...
        if (!row) continue;
        const summary = buildSummary({ ...row, model }, entry.prepared);
        if (!summary) continue;
        putCache(entry.key, summary);
        results[entry.batchIndex] = { index: entry.batchIndex, url: entry.prepared.url, mode: entry.prepared.mode, summary };
        generated += 1;
      }
    } catch (error) {
      console.warn('[summary-batch-v2188] generate failed', error?.message || error);
    }
  }
  ...
  return res.status(200).json({ results, batch: 'v2188', generated, cached: cachedCount });
}
