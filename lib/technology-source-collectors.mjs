...
const DIRECT_RSS = [
  { sourceType: 'プレスリリース', sourceLabel: 'PR TIMES', urls: ['https://prtimes.jp/index.rdf'], maxItems: 28 },
  { sourceType: '専門Webメディア', sourceLabel: '家電 Watch', urls: ['https://kaden.watch.impress.co.jp/data/rss/1.0/kdw/feed.rdf'], maxItems: 28 },
  { sourceType: '専門Webメディア', sourceLabel: 'MONOist', urls: ['https://rss.itmedia.co.jp/rss/2.0/monoist.xml'], maxItems: 28 },
  { sourceType: '専門Webメディア', sourceLabel: 'Gizmodo Japan', urls: ['https://www.gizmodo.jp/index.xml'], maxItems: 28 },
  {
    sourceType: 'クラウドファンディング', sourceLabel: 'Kickstarter', maxItems: 12,
    urls: [
  ...
  { sourceType: '研究機関PR', sourceLabel: 'EurekAlert!', urls: ['https://www.eurekalert.org/rss.xml'], maxItems: 28 }
];
...
async function collectRssConfig(config) {
  const settled = await Promise.allSettled((config.urls || []).map(url =>
    fetchRssSource({ name: config.sourceLabel, url, maxItems: config.maxItems || 24 })
  ));
  const items = [];
  const errors = [];
  ...
  const ranked = items
    ...
    .slice(0, config.sourceLabel === 'Kickstarter' ? 16 : 12);
...
async function collectSciencePortal() {
  ...
  try {
    ...
    try {
      ...
      return {
        items: items.sort((a, b) => relevanceScore(b) - relevanceScore(a) || dateValue(b.pubDate) - dateValue(a.pubDate)).slice(0, 12),
        errors: []
      };
    } finally {
      dom.window.close();
...
async function collectEpoOps() {
  ...
  return {
    items: items.filter(item => {
      ...
      seen.add(key);
      return true;
    }).sort((a, b) => relevanceScore(b) - relevanceScore(a) || dateValue(b.pubDate) - dateValue(a.pubDate)).slice(0, 12),
    errors
  };
}
...
export async function collectTechnologySourceCandidates() {
  ...
  return {
    candidates: unique.sort((a, b) => relevanceScore(b) - relevanceScore(a) || dateValue(b.pubDate) - dateValue(a.pubDate)).slice(0, 80),
    missingMedia,
    status,
    errors
  };
