...
const APP_SHELL=[
  ...
  '/src/features/reader/news-today.js','/src/features/reader/reader.js','/src/features/reader/reader-data.js','/src/features/reader/reader-flow.js','/src/features/reader/reader-focus.js','/src/features/reader/reader-rank.js','/src/features/reader/reader-recommendations.js','/src/features/reader/reader-attention.js','/src/features/reader/summary-request-coordinator.js','/src/features/reader/summary-fetch-gate.js','/src/features/reader/summary-instant-ux.js','/src/features/reader/reader-summary-accelerator.js','/src/features/reader/reader-deep-dive-page-unit.js','/src/features/reader/reader-deep-dive-long-wait.js','/src/features/reader/reader-recommend-loading-ui.js','/src/features/reader/reader-recommend-latest-labels.js','/src/features/reader/reader-recommend-cache-fast.js','/src/features/reader/reader-list-ux.js',
...
async function instagramMediaResponse(request,url){const cache=await caches.open(INSTAGRAM_MEDIA_CACHE);const canonical=instagramMediaCanonical(url);const refresh=url.searchParams.get('refresh')==='1';if(!refresh){const hit=await cache.match(canonical);if(hit)return hit;}const source=url.searchParams.get('src')||'';if(!allowedInstagramMediaSource(source)){const fallback=await cache.match(canonical);return fallback||new Response('',{status:400,statusText:'Invalid Instagram media source'});}try{const response=await fetch(source,{mode:'no-cors',credentials:'omit',cache:'force-cache',referrerPolicy:'no-referrer'});try{await cache.put(canonical,response.clone());trimInstagramMediaCache(cache);}catch{}return response;}catch{const fallback=await cache.match(canonical);return fallback||new Response('',{status:503,statusText:'Instagram media unavailable'});}}
self.addEventListener('message',event=>{if(event.data?.type==='SKIP_WAITING'){self.skipWaiting();return;}if(event.data?.type==='IG_MEDIA_EVICT'){const path=String(event.data?.path||'');if(!path.startsWith(INSTAGRAM_MEDIA_PREFIX))return;event.waitUntil(caches.open(INSTAGRAM_MEDIA_CACHE).then(cache=>cache.delete(new Request(new URL(path,self.location.origin).href))).catch(()=>false));}});
...
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin===self.location.origin&&url.pathname.startsWith(INSTAGRAM_MEDIA_PREFIX)){event.respondWith(instagramMediaResponse(request,url));return;}if(url.origin!==self.location.origin||url.pathname.startsWith('/api/')){event.respondWith(fetch(request).catch(()=>new Response('',{status:503,statusText:'Service Unavailable'})));return;}if(request.mode==='navigate'||url.pathname.endsWith('.js')||url.pathname.endsWith('.mjs')||url.pathname.endsWith('.css')||url.pathname==='/index.html'||url.pathname==='/'){event.respondWith(networkFirst(request));return;}event.respondWith(cachedFallback(request).then(cached=>cached||fetch(request).then(response=>{if(response?.ok){const clone=response.clone();caches.open(CACHE_NAME).then(cache=>cacheStatic(cache,request,clone)).catch(()=>{});}return response;})));});
